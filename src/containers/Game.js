...
import { checkWord, sortWordQueue } from "../config/PhrasesCheck";
import {
  saveHighScore,
  getHighScore,
  scoreForThisWord
} from "../config/SaveScore";
import { lettersAdjustedPerWeight } from "../config/GenerateWord";
        ...
        if (_l.isWord) this.wordQueue.push(letter);
          ...
          this.wordQueue.splice(
            this.wordQueue.findIndex(
              (_l) =>
                _l && _l.pos.x === letter.pos.x && _l.pos.y === letter.pos.y
            ),
            1
          );
        }
    ...
    if (this.wordQueue.length > 0) {
      ...
      if (this.wordQueue.length > 1) {
        for (let i = 0; i < this.wordQueue.length - 1; i++) {
          if (
            Math.abs(this.wordQueue[i].pos.x - this.wordQueue[i + 1].pos.x) !==
            1
          ) {
            wordIsInRow = false;
          }
      ...
      if (!wordIsInRow) {
        ...
        for (let i = 0; i < this.wordQueue.length - 1; i++) {
          if (
            Math.abs(this.wordQueue[i].pos.y - this.wordQueue[i + 1].pos.y) !==
            1
          ) {
      ...
      <div className={css(styles.container)}>
        <div className={css(styles.scoreLine)}>
          ...
          {this.nextLetter && (
            <div className={css(styles.score)}>
              {" "}
              {`Next : ${this.nextLetter.toUpperCase()}`}{" "}
            </div>
          )}
        </div>
        {this.gameState !== GAMESTATE.ENDED && (
